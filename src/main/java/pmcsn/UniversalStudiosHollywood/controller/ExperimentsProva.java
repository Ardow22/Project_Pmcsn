...
public class ExperimentsProva {
	...
	private double[] finiteHorizonSimulation(long seed, Rngs rng, TransientStats ts, String expFilename) {
    	 ...
    	 // Node area sdoppiate per le attrazioni
            ...
            if (e == INDEX_CHANGE_LAMBDA) {
            	...
            	if (totalJobsMario <= SERVERS_MARIO) { //verifico se posso essere servito subito
            		...
            		// Accumulo del tempo di servizio distinto[cite: 1]
     ...
     /* Calcolo dei Tempi Medi di Risposta (E[T]) per l'intera durata della run
        double avgSicurezza = (totalSicurezzaCheck > 0) ? nodeAreaSicurezza / totalSicurezzaCheck : 0.0;
        double avgBiglietteria = (totalBiglietteriaCheck > 0) ? nodeAreaBiglietteria / totalBiglietteriaCheck : 0.0;
        double avgControlli = (totalControlliCheck > 0) ? nodeAreaControlli / totalControlliCheck : 0.0;

        double avgMarioStandard = (totalMarioStandardCheck > 0) ? nodeAreaMarioStandard / totalMarioStandardCheck : 0.0;
        double avgMarioExpress = (totalMarioExpressCheck > 0) ? nodeAreaMarioExpress / totalMarioExpressCheck : 0.0;
        double avgHPStandard = (totalHPStandardCheck > 0) ? nodeAreaHPStandard / totalHPStandardCheck : 0.0;
        double avgHPExpress = (totalHPExpressCheck > 0) ? nodeAreaHPExpress / totalHPExpressCheck : 0.0;*/
     ...
     //--- 1. Accumulo del tempo di servizio totale erogato dai serventi per nodo ---
        ...
        /*double queueAreaMarioStandard = nodeAreaMarioStandard;
        for (int i = INDEX_FIRST_SERVER_MARIO_VERIFICA; i <= INDEX_LAST_SERVER_MARIO_VERIFICA; i++) {
            queueAreaMarioStandard -= sum[i].service;
        }
        
        double queueAreaMarioExpress = nodeAreaMarioExpress;
        for (int i = INDEX_FIRST_SERVER_MARIO_VERIFICA; i <= INDEX_LAST_SERVER_MARIO_VERIFICA; i++) {
            queueAreaMarioExpress -= sum[i].service;
        }

        double queueAreaHPStandard = nodeAreaHPStandard;
        for (int i = INDEX_FIRST_SERVER_HP_VERIFICA; i <= INDEX_LAST_SERVER_HP_VERIFICA; i++) {
            queueAreaHPStandard -= sum[i].service;
        }
        
        double queueAreaHPExpress = nodeAreaHPExpress;
        for (int i = INDEX_FIRST_SERVER_HP_VERIFICA; i <= INDEX_LAST_SERVER_HP_VERIFICA; i++) {
            queueAreaHPExpress -= sum[i].service;
        }*/
